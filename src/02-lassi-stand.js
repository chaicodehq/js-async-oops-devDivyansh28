...
LassiStand.prototype.takeOrder = function(customerName , flavor , quantity){
  ...
  let item = this.menu.find((item)=>item.flavor===flavor);
  if(item){
    const newObject = {id : this._nextOrderId++, customer:customerName,flavor,quantity, total: item.price*quantity, status : "pending"}
...
LassiStand.prototype.completeOrder = function(orderId){
  const findedItem = this.orders.find((item)=>item.id===orderId)
  if(findedItem){
    if(findedItem.status==="pending"){
...
LassiStand.prototype.getMenu = function(){
  let copiedMenu = structuredClone(this.menu);
  return copiedMenu;
}
// LassiStand.prototype.addFlavor = function(flavor, price) { ... }
// LassiStand.prototype.takeOrder = function(customerName, flavor, quantity) { ... }
